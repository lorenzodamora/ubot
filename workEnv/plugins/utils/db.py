...
from aiosqlite import connect as aconnect, Row as Arow, Cursor, OperationalError as AoperationalError
from .db_name import dbn
...
class SqliteDatabase(Database):
    def __init__(self, file):
        self._conn = connect(file, check_same_thread=False)
...
class AioSqliteDatabase(Database):
    def __init__(self, file=dbn):
        self._file = file
        self._conn = None

    async def _connect(self):
        self._conn = await aconnect(self._file)
        self._conn.row_factory = Arow
        self._cursor = await self._conn.cursor()
        self._lock = Alock()

    @staticmethod
    def _parse_row(row: Arow):
        parse_func = {
            "bool": lambda x: x == "1",
            "int": int,
            "str": lambda x: x,
    ...
    async def _execute(self, module: str, *args, **kwargs) -> Cursor:
        try:
            return await self._cursor.execute(*args, **kwargs)
        except AoperationalError as e:
            if str(e).startswith("no such table"):
